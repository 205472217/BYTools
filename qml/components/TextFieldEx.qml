...
TextField {
    ...
    readonly property var _intValidator: IntValidator {
        bottom: root.minNumber; top: root.maxNumber
    }
    readonly property var _doubleValidator: DoubleValidator {
        bottom: root.minNumber; top: root.maxNumber; decimals: 4
        ...
        if (clamped !== val) {
            ...
            if (val > Number(maxNumber))
                ...
                for (var i = 0; i < t.length; i++) {
                    var c = t.charAt(i)
                    if (c >= '1' && c <= '9') { onlyZero = false; break }
                }
                if (!onlyZero)
                    text = editType === 1 ? Math.floor(clamped).toString() : clamped.toString()
            }
    ...
    readonly property color _highlightBorderColor:
        paletteGroup ? (_p[paletteGroup + "_highlightBorderColor"] || focusBorderColor) : focusBorderColor
    ...
    background: Rectangle {
        ...
        border.color: root.activeFocus ? root._focusBorderColor :
                      root.highlightOnValid ? root._highlightBorderColor :
